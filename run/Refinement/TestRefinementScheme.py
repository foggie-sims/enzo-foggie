...
def test_multi_refinement_regions():
    ds = yt.load(os.path.join(_dir_name, 'DD0006/DD0006'))
    data  = ds.all_data()
    ...
    tfrac = (current_time.value-t[0])/(t[1]-t[0])
    ...
    # First check that our regions are actually large enough to actually contain a full cell
    assert(len(MRR1['index','cell_volume'])>0)
    assert(len(MRR2['index','cell_volume'])>0)
    # Then check that they only contain cells that are refined to the minimum level
    ...
    # We are ONLY refining by MultiRefineRegion, so check that refinement isn't happening 
    # too much outside of these regions. Some spread is inevitable 
    FullRegion = ds.box([0,0,0],[1,1,1])
    NoMRR = FullRegion-(MRR1+MRR2)
    NoMRR_refined = NoMRR['index','cell_volume']>cellvolumes[0]
    assert(len(NoMRR['index','cell_volume'][NoMRR_refined])/len(NoMRR['index','cell_volume'])<0.55) 
