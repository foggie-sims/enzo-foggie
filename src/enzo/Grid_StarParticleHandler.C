...
extern "C" void FORTRAN_NAME(star_maker1)(int *nx, int *ny, int *nz,
		 ...
		 int *np,
             ...
             float *mp, float *tdp, float *tcp, int *iminit, float *minit);
#endif /* STAR1 */
...
extern "C" void FORTRAN_NAME(star_maker_h2reg)(int *nx, int *ny, int *nz,
	     ...
	     int *UseLocalDensityMax, 
	     int *WriteStarLogFiles, 
...
extern "C" void FORTRAN_NAME(star_feedback3)(int *nx, int *ny, int *nz,
	     ...
	     int *distrad, int *diststep, int *distcells,
             ...
             FLOAT *xp, FLOAT *yp, FLOAT *zp, float *up, float *vp, float *wp,
             float *mp, float *tdp, float *tcp, float *metalf, int *type,
			float *justburn, int *iminit, float *minit, int *crmodel, float *crfeedback, float *cr);
  ...
  if (StarParticleCreation > 0) {
    ...
    if (STARMAKE_METHOD(MOM_STAR)) {
      ...
      FORTRAN_NAME(star_maker3mom)(
       ...
       tg->ParticleAttribute[2],
       &StarMakerTypeIaSNe, BaryonField[MetalIaNum], tg->ParticleAttribute[3],
       &StarMakerExplosionDelayTime, &StarMakerStoreInitialMass, tg->ParticleInitialMass);
    ...
    if (STARMAKE_METHOD(SINGLE_SUPERNOVA)) {
      ...
      if (level == MaximumRefinementLevel)
      {
        ...
        FORTRAN_NAME(star_maker_ssn)(
          ...
          &StarMakerMaximumFractionCell, &StarMakerStoreInitialMass, tg->ParticleInitialMass);
      }
    ...
    if (STARMAKE_METHOD(DISTR_FEEDBACK)) {
      ...
      FORTRAN_NAME(star_maker10)(
       ...
       tg->ParticleVelocity[0], tg->ParticleVelocity[1],
          tg->ParticleVelocity[2],
       tg->ParticleMass, tg->ParticleAttribute[1], tg->ParticleAttribute[0],
          tg->ParticleAttribute[2], &StarMakerStoreInitialMass, tg->ParticleInitialMass);
  ...
  if (STARFEED_METHOD(MECH_STAR)) {
	...
	for (i = GridStartIndex[0]; i <= GridEndIndex[0]; i++) {
	  ...
	  if (MultiSpecies == 0) {
	    ...
	    mu_field[index] = BaryonField[DeNum][index] + BaryonField[HINum][index] + BaryonField[HIINum][index] +
	      (BaryonField[HeINum][index] + BaryonField[HeIINum][index] + BaryonField[HeIIINum][index])/4.0 +
         BaryonField[MetalNum][index]*BaryonField[DensNum][index]/16.0;
	    if (MultiSpecies > 1) {
	      mu_field[index] += BaryonField[HMNum][index] + (BaryonField[H2INum][index] + BaryonField[H2IINum][index])/2.0;
	    }
	    if (MultiSpecies > 2) {
...
#define NO_PARTICLE_IN_GRID_CHECK   
    ...
    //---- MODIFIED SF ALGORITHM (NO-JEANS MASS, NO dt DEPENDENCE, NO STOCHASTIC SF)
      ...
      FORTRAN_NAME(star_feedback7)(
       ...
       ParticleMass, ParticleAttribute[1], ParticleAttribute[0],
          ParticleAttribute[2], ParticleType, &RadiationData.IntegratedStarFormation, 
       &MBHParticleType, &mbhradius, &StarMakerStoreInitialMass, ParticleInitialMass);
